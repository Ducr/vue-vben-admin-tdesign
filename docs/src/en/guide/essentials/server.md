...
From the browser's console Network tab, the request appears as `http://localhost:5173/api/user`. This is because the proxy configuration does not change the local request's URL.
...
```bash
VITE_GLOB_API_URL=https://ducrong.com/vue-vben-admin-tdesign/api
```
