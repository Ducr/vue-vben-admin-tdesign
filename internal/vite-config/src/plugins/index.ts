...
// 动态导入，避免在 VitePress 环境中加载 webpack 依赖
const loadVueI18nPlugin = async () => {
  const { default: viteVueI18nPlugin } = await import('@intlify/unplugin-vue-i18n/vite');
...
async function loadApplicationPlugins(
  ...
  return await loadConditionPlugins([
    ...commonPlugins,
    {
      condition: i18n,
      plugins: async () => {
        const viteVueI18nPlugin = await loadVueI18nPlugin();
        return [
          viteVueI18nPlugin({
